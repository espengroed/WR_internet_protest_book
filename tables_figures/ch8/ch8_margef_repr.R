...
p <- ggplot(dfplot, aes(x=x, y=means, ymin=sdLower, ymax=sdUpper)) + 
  ...
  geom_rug(sides="b") + coord_cartesian(ylim = c(-0.002,.005)) +
  theme(axis.text=element_text(size=14), axis.title=element_text(size=15,face="bold"))

p

pdf("ch8/cityweek_diffusion_bayes_sendingeffect_time_repr0.pdf")
...
pdf("ch8/cityweek_diffusion_bayes_sendingeffect_time_repr1.pdf")
par(mar=c(4.5,4.5,1,1))
p
dev.off()
